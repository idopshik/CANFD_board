...
int calculete_prsc_and_perio(int val, float *arr){
	...
	int peri =  (int)((APB1_CLK / (val * factor * (presc + 1))) - 1);  // значение регистра ARR
...
static void MX_TIM3_Init(void) {
	...
	if (HAL_TIM_OC_ConfigChannel(&htim3, &sConfigOC, TIM_CHANNEL_4) !=
	    HAL_OK) {
		Error_Handler();
	}
...
static void MX_TIM5_Init(void) {
	...
	sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
	sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
	if (HAL_TIM_OC_ConfigChannel(&htim5, &sConfigOC, TIM_CHANNEL_1) !=
	    HAL_OK) {
		Error_Handler();
	}
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan,
	...
	if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET) {
		...
		if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &RxHeader1,
					   canRX) != HAL_OK) {
			/* Reception Error */
			Error_Handler();
		} else {
			if ((RxHeader1.Identifier != CAN_SPECIAL_ID)) {
				HAL_GPIO_TogglePin(LED_TX3_GPIO_Port,
						   LED_TX3_Pin);
				my_printf("wrongID: %#x \n\r",
